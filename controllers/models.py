...
class Pokemon(Parsable):
    ...
    def process_json(self, json):
        try:
            self.name = json.get(POKEMON_NAME_KEY)
            self.__url = json.get(POKEMON_URL_KEY)
            self.__ens_id = json.get(POKEMON_ENS_ID_KEY)
            self.__generation = json.get(POKEMON_GENERATION_KEY)
        except ValueError:
            print(HR_ERROR_JSON_DECODE)

    def __str__(self):
        return f'{self.name} | gen {self.__generation}\n{self.__ens_id}\n{self.__url}'
